...
export function initializePlayerEvents(player, audioPlayer, scrobbler, ui) {
    ...
    if (player.smartShuffleActive) {
        shuffleBtn.classList.add('smart');
        shuffleBtn.title = 'Smart Shuffle';
    }
    ...
    shuffleBtn.addEventListener('click', async () => {
        ...
        shuffleBtn.classList.toggle('smart', player.smartShuffleActive);
        
        if (player.smartShuffleActive) {
             shuffleBtn.title = 'Smart Shuffle';
        } else if (player.shuffleActive) {
    ...
    mainContent.addEventListener('click', async (e) => {
        ...
        if (trackItem && !trackItem.dataset.queueIndex && !e.target.closest('.remove-from-playlist-btn')) {
            ...
            if (trackList.length > 0) {
                ...
                player.setQueue(trackList, startIndex >= 0 ? startIndex : 0);
                document.getElementById('shuffle-btn').classList.remove('active', 'smart');
                player.playTrackFromQueue();
            }
        }
