...
export class UIRenderer {
    constructor(api, player) {
        ...
        authManager.onAuthStateChanged(() => {
            ...
            // Update top nav profile avatar
            this.updateTopNavProfile();
        });

        // Initial profile avatar update
        this.updateTopNavProfile();
    }

    updateTopNavProfile() {
        const profileBtn = document.getElementById('top-nav-profile-btn');
        ...
        if (user) {
            ...
            const photoUrl = user.photoURL || `https://api.dicebear.com/9.x/lorelei/svg?seed=${encodeURIComponent(name)}`;
    ...
    async renderLibraryPage() {
        ...
        const smartShuffleBtn = document.getElementById('smart-shuffle-liked-tracks-btn');
        const sortSelect = document.getElementById('library-tracks-sort');

        // Sort function for tracks
        const sortTracks = (tracks, sortBy) => {
            const sorted = [...tracks];
            switch (sortBy) {
                case 'added-asc':
        ...
        const renderSortedTracks = (tracks) => {
            const sortBy = sortSelect?.value || 'added-desc';
            const sortedTracks = sortTracks(tracks, sortBy);
            this.renderListWithTracks(tracksContainer, sortedTracks, true);
        };

        // Year filter functionality
        const yearFilter = document.getElementById('library-tracks-year-filter');
        const filterByYear = (tracks, year) => {
            if (year === 'all') return tracks;
            return tracks.filter(t => {
                const releaseDate = t.album?.releaseDate || t.streamStartDate;
                if (!releaseDate) return false;
                const trackYear = new Date(releaseDate).getFullYear();
                return trackYear === parseInt(year);
            });
        };

        // Get unique years from tracks
        const getUniqueYears = (tracks) => {
            const years = new Set();
            tracks.forEach(t => {
                const releaseDate = t.album?.releaseDate || t.streamStartDate;
                if (releaseDate) {
        ...
        const renderFilteredAndSortedTracks = (tracks) => {
            // Query DOM directly to avoid stale references after element replacement
            const currentSortSelect = document.getElementById('library-tracks-sort');
            const currentYearFilter = document.getElementById('library-tracks-year-filter');
            const sortBy = currentSortSelect?.value || 'added-desc';
            const yearValue = currentYearFilter?.value || 'all';
            let filtered = filterByYear(tracks, yearValue);
            const sorted = sortTracks(filtered, sortBy);
            this.renderListWithTracks(tracksContainer, sorted, true);

            // Update bulk mode if active
            if (document.getElementById('bulk-actions-toolbar')?.style.display === 'flex') {
                addBulkCheckboxes();
            }
        };
        ...
        const addBulkCheckboxes = () => {
            tracksContainer.querySelectorAll('.track-item').forEach(item => {
                ...
                // Hide the original content (track number or cover image)
                Array.from(trackNumber.childNodes).forEach(child => {
                    if (child.nodeType === Node.ELEMENT_NODE) {
                        ...
                        trackNumber.replaceChild(span, child);
                    }
                });
                ...
                checkbox.checked = selectedTrackIds.has(item.dataset.trackId);
        ...
        const removeBulkCheckboxes = () => {
            tracksContainer.querySelectorAll('.track-item').forEach(item => {
                ...
                // Remove checkbox
                const checkbox = trackNumber.querySelector('.bulk-checkbox');
                if (checkbox) checkbox.remove();

                // Restore visibility of original content
                Array.from(trackNumber.children).forEach(child => {
                    if (child.classList.contains('track-number-text')) {
                        // Convert span back to text node
                        const textNode = document.createTextNode(child.textContent);
                        trackNumber.replaceChild(textNode, child);
                    } else {
                        child.style.display = '';
                    }
                });
        ...
        const updateSelectedCount = () => {
            ...
            if (countEl) countEl.textContent = `${selectedTrackIds.size} selected`;
            
            const selectAllCheckbox = document.getElementById('select-all-tracks');
            if (selectAllCheckbox) {
                const allCheckboxes = tracksContainer.querySelectorAll('.bulk-checkbox');
                selectAllCheckbox.checked = allCheckboxes.length > 0 && selectedTrackIds.size === allCheckboxes.length;
                selectAllCheckbox.indeterminate = selectedTrackIds.size > 0 && selectedTrackIds.size < allCheckboxes.length;
            }
        };
        ...
        const initBulkActions = () => {
            ...
            if (selectAllCheckbox) {
                selectAllCheckbox.onchange = (e) => {
                    ...
                    allCheckboxes.forEach(cb => {
                        ...
                        const trackId = cb.closest('.track-item')?.dataset.trackId;
                        if (trackId) {
            ...
            if (bulkDownload) {
                bulkDownload.onclick = async () => {
                    ...
                    const { downloadTracks, showNotification } = await import('./downloads.js');
                    const { downloadQualitySettings } = await import('./storage.js');
                    const selectedTracks = likedTracks.filter(t => selectedTrackIds.has(String(t.id)));
                    downloadTracks(selectedTracks, this.api, downloadQualitySettings.getQuality());
            ...
            if (bulkRemove) {
                bulkRemove.onclick = async () => {
                    if (selectedTrackIds.size === 0) return;
                    if (!confirm(`Remove ${selectedTrackIds.size} track(s) from liked?`)) return;
                    const { showNotification } = await import('./downloads.js');
                    const { syncManager } = await import('./accounts/firestore.js');
                    
                    for (const id of selectedTrackIds) {
                        const track = likedTracks.find(t => String(t.id) === id);
                        if (track) {
                            await db.toggleFavorite('track', track);
                            syncManager.syncLibraryItem('track', track, false);
                        }
                    }
                    showNotification(`Removed ${selectedTrackIds.size} tracks from liked`);
                    selectedTrackIds.clear();
            ...
            if (bulkAddPlaylist) {
                bulkAddPlaylist.onclick = async () => {
                    ...
                    if (playlists.length === 0) {
                        showNotification('No playlists. Create one first.');
                        return;
                    }
                    ...
                    modal.innerHTML = `
                        ...
                        <div class="modal-content">
                            <h3>Add ${selectedTrackIds.size} tracks to playlist</h3>
                            <div class="modal-list">
                                ${playlists.map(p => `<div class="modal-option" data-id="${p.id}">${p.name}</div>`).join('')}
                            </div>
                    ...
                    modal.addEventListener('click', async (e) => {
                        ...
                        if (option) {
                            ...
                            for (const id of selectedTrackIds) {
                                const track = likedTracks.find(t => String(t.id) === id);
                                if (track) {
                                    const result = await db.addTrackToPlaylist(playlistId, track);
                            ...
                            let msg = `Added ${addedCount} track(s) to ${option.textContent}`;
        ...
        if (likedTracks.length) {
            ...
            if (smartShuffleBtn) smartShuffleBtn.style.display = 'flex';
            
            // Populate year filter
            if (yearFilter) {
                const years = getUniqueYears(likedTracks);
                yearFilter.innerHTML = '<option value="all">All Years</option>' + 
                    years.map(y => `<option value="${y}">${y}</option>`).join('');
            }
            
            renderFilteredAndSortedTracks(likedTracks);

            // Add sort event listener (remove old one first)
            if (sortSelect) {
            ...
            // Add year filter listener
            if (yearFilter) {
                const newYearFilter = yearFilter.cloneNode(true);
                // Preserve the populated options
                newYearFilter.innerHTML = yearFilter.innerHTML;
                yearFilter.parentNode.replaceChild(newYearFilter, yearFilter);
                newYearFilter.addEventListener('change', () => {
                    renderFilteredAndSortedTracks(likedTracks);
                });
            }
            ...
            if (smartShuffleBtn) smartShuffleBtn.style.display = 'none';
        ...
        if (statsContainer) {
            const parts = [];
            if (likedTracks.length) parts.push(`${likedTracks.length} tracks`);
            if (likedAlbums.length) parts.push(`${likedAlbums.length} albums`);
            if (likedArtists.length) parts.push(`${likedArtists.length} artists`);
            statsContainer.textContent = parts.length ? parts.join(' · ') : 'Start adding favorites';
        }
    ...
    async renderHomeSongs(forceRefresh = false) {
        ...
        const prevBtn = document.getElementById('songs-carousel-prev');
        const nextBtn = document.getElementById('songs-carousel-next');
        
        if (songsContainer) {
            if (forceRefresh) {
                songsContainer.innerHTML = Array(10).fill(this.createSkeletonTrack(true)).join('');
            } else if (songsContainer.children.length > 0 && !songsContainer.querySelector('.skeleton')) {
            ...
            try {
                ...
                if (filteredTracks.length > 0) {
                    // Render tracks in carousel grid format
                    songsContainer.innerHTML = filteredTracks.map((track, idx) => 
                        this.createTrackItemHTML(track, idx + 1, true)
                    ).join('');
                    
                    // Store track data and update like states
                    filteredTracks.forEach((track, index) => {
                        const el = songsContainer.children[index];
                        if (el) {
                    ...
                    this.initSongsCarousel(songsContainer, prevBtn, nextBtn);
                } else {
    ...
    initSongsCarousel(container, prevBtn, nextBtn) {
        if (!container) return;
        ...
        const updateButtons = () => {
            ...
            if (prevBtn) {
                prevBtn.style.display = hasContent ? 'flex' : 'none';
                prevBtn.style.opacity = atStart ? '0.3' : '1';
                prevBtn.disabled = atStart;
            }
            if (nextBtn) {
                nextBtn.style.display = hasContent ? 'flex' : 'none';
                            ...
                            if (trackData) {
                                try {
                                    ...
                                    if (playlistMatch) {
                                        ...
                                        const result = await db.addTrackToPlaylist(playlistId, trackData);
                                        
                                        if (result?.isDuplicate) {
                                            const { showNotification } = await import('./downloads.js');
                                            showNotification('Track already exists in playlist');
                                            return;
                                        }
    ...
    async renderPlaylistPage(playlistId, source = null) {
        ...
        try {
            ...
            if (playlistData) {
                ...
                document.title = `${playlistData.name || playlistData.title} - Loop`;
            } else {
    ...
    updatePlaylistHeaderActions(playlist, isOwned, tracks, showShare = false) {
        ...
        ['shuffle-playlist-btn', 'smart-shuffle-playlist-btn', 'edit-playlist-btn', 'delete-playlist-btn', 'share-playlist-btn'].forEach((id) => {
        ...
        // Smart Shuffle
        const smartShuffleBtn = document.createElement('button');
        smartShuffleBtn.id = 'smart-shuffle-playlist-btn';
        smartShuffleBtn.className = 'btn-primary';
        smartShuffleBtn.title = 'Smart Shuffle';
        smartShuffleBtn.innerHTML =
        ...
        if (isOwned) {
            ...
            exportBtn.innerHTML =
                '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17,8 12,3 7,8"/><line x1="12" y1="3" x2="12" y2="15"/></svg><span>Export</span>';
            exportBtn.onclick = async () => {
                const { showExportModal } = await import('./playlist-export.js');
                showExportModal({ ...playlist, tracks });
            };
            fragment.appendChild(exportBtn);

            // Merge playlists button
            const mergeBtn = document.createElement('button');
            mergeBtn.id = 'merge-playlist-btn';
            mergeBtn.className = 'btn-secondary';
            mergeBtn.innerHTML =
                '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 6l4 -4l4 4"/><path d="M12 2v10.3a4 4 0 0 1 -1.172 2.828l-4.828 4.872"/><path d="M12 12.3a4 4 0 0 0 1.172 2.828l4.828 4.872"/></svg><span>Merge</span>';
            mergeBtn.onclick = async () => {
                ...
                if (otherPlaylists.length === 0) {
                    const { showNotification } = await import('./downloads.js');
                    showNotification('No other playlists to merge with');
                    return;
                }
                ...
                modal.innerHTML = `
                    <div class="modal-overlay"></div>
                    <div class="modal-content">
                        <h3>Merge into "${playlist.name}"</h3>
                        <p style="margin-bottom: 1rem; color: var(--muted-foreground); font-size: 0.9rem;">
                            Select a playlist to merge. All tracks from the selected playlist will be added to this one.
                        </p>
                        <div class="modal-list">
                            ${otherPlaylists.map(p => `
                                <div class="modal-option" data-id="${p.id}" style="padding: 0.75rem; cursor: pointer; border-radius: var(--radius); display: flex; justify-content: space-between; align-items: center;">
                                    <span>${p.name}</span>
                                    <span style="color: var(--muted-foreground); font-size: 0.85rem;">${p.numberOfTracks || 0} tracks</span>
                                </div>
                            `).join('')}
                        </div>
                        <div class="modal-actions">
                ...
                modal.querySelectorAll('.modal-option').forEach(opt => {
                    opt.addEventListener('mouseenter', () => opt.style.background = 'var(--secondary)');
                ...
                modal.addEventListener('click', async (e) => {
                    ...
                    if (option) {
                        ...
                        const sourcePlaylist = await db.getPlaylist(sourceId);
                        if (sourcePlaylist && sourcePlaylist.tracks) {
                            let addedCount = 0, duplicateCount = 0;
                            for (const track of sourcePlaylist.tracks) {
                                const result = await db.addTrackToPlaylist(playlist.id, track);
                                if (result?.isDuplicate) duplicateCount++;
                                else addedCount++;
                            }
                            
                            const { showNotification } = await import('./downloads.js');
                            let msg = `Merged ${addedCount} track(s) from "${sourcePlaylist.name}"`;
                            if (duplicateCount > 0) msg += ` (${duplicateCount} duplicates skipped)`;
                            showNotification(msg);
                            
                            syncManager.syncUserPlaylist(await db.getPlaylist(playlist.id), 'update');
                            
                            // Refresh the playlist page
                            const scrollTop = document.querySelector('.main-content')?.scrollTop || 0;
                            await this.renderPlaylistPage(playlist.id, 'user');
    ...
    async renderAccountPage() {
        ...
        if (user) {
            // Logged In Design (Spotify Style)
            const name = user.displayName || user.email.split('@')[0];
            const photoUrl = user.photoURL || `https://api.dicebear.com/9.x/lorelei/svg?seed=${encodeURIComponent(name)}`;
            
            // Get playlists for count and display
            let publicPlaylists = [];
            try {
                publicPlaylists = await db.getPlaylists();
            } catch (e) {
            ...
            container.innerHTML = `
                <div class="spotify-header">
                    <div class="header-image">
                        <img src="${photoUrl}" alt="${escapeHtml(name)}" style="width:100%; height:100%; border-radius:50%; object-fit:cover;" />
                    </div>
                    <div class="header-info">
                        <span class="meta-label">Profile</span>
                        <h1 class="header-title">${escapeHtml(name)}</h1>
                ...
                <div class="profile-actions-bar">
                   <!-- Removed Edit and Menu buttons as requested -->
                   <button id="profile-signout-btn" class="action-btn" style="border: 1px solid rgba(255,255,255,0.3);">Sign Out</button>
                </div>

                <div class="content-section">
                    <h2>Public Playlists</h2>
                    <div class="playlist-grid" id="profile-playlists-grid">
             ...
             // Render Playlists
             const grid = container.querySelector('#profile-playlists-grid');
             if (publicPlaylists.length === 0) {
                 grid.innerHTML = '<p style="color:var(--text-secondary)">No public playlists yet.</p>';
             } else {
                 publicPlaylists.forEach(pl => {
                     const card = document.createElement('div');
                     card.className = 'playlist-card spotify-card';
                     card.onclick = () => window.location.hash = `#userplaylist/${pl.id}`;
                     
                     // Cover logic (use first track or placeholder)
                     let coverHtml = `<div class="placeholder-cover" style="width:100%; height:100%; background:#333; display:flex; align-items:center; justify-content:center"><svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#666" stroke-width="2"><path d="M9 18V5l12-2v13"></path><circle cx="6" cy="18" r="3"></circle><circle cx="18" cy="16" r="3"></circle></svg></div>`;
                     
                     if (pl.tracks.length > 0 && pl.tracks[0].albumArt) {
                         coverHtml = `<img src="${pl.tracks[0].albumArt}" loading="lazy" style="width:100%; height:100%; object-fit:cover;" />`;
                     }
                     ...
                     card.innerHTML = `
                        ...
                        <div class="card-info">
                            ...
                            <p class="card-desc">By ${escapeHtml(name)}</p>
                        </div>
                     `;
             ...
             // Attach listeners
             container.querySelector('#profile-signout-btn').addEventListener('click', () => authManager.signOut());
            ...
            const toggleBtn = container.querySelector('#auth-email-toggle-btn');
            const cancelBtn = container.querySelector('#auth-email-cancel-btn');
            ...
            signinBtn.addEventListener('click', () => {
                authManager.signInWithEmail(emailInput.value, passInput.value);
            });
            
            signupBtn.addEventListener('click', () => {
                authManager.signUpWithEmail(emailInput.value, passInput.value);
            });
        }
    }
}
