...
export function initializeUIInteractions(player, api) {
                ...
                modal.addEventListener('click', async (e) => {
                    ...
                    if (option) {
                        ...
                        try {
                            ...
                            let message = `Added ${addedCount} track${addedCount !== 1 ? 's' : ''} to ${playlistName}`;
                            if (duplicateCount > 0) {
    ...
    document.querySelectorAll('.library-tab').forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            page.querySelectorAll('.library-tab').forEach((t) => t.classList.remove('active'));
            page.querySelectorAll('.search-tab-content').forEach((c) => c.classList.remove('active'));

            tab.classList.add('active');

            const contentId = `library-tab-${tab.dataset.tab}`;
            document.getElementById(contentId)?.classList.add('active');
        });
    });
}
