...
import { checkRequiredConfig } from './setup.js';
import {
    apiSettings,
    themeManager,
    nowPlayingSettings,
...
function initializeKeyboardShortcuts(player, audioPlayer) {
    document.addEventListener('keydown', (e) => {
        ...
        switch (e.key.toLowerCase()) {
            ...
            case '/':
                e.preventDefault();
                (document.getElementById('top-search-input') || document.getElementById('search-input'))?.focus();
                break;
            case 'escape':
                document.getElementById('search-input')?.blur();
                document.getElementById('top-search-input')?.blur();
...
document.addEventListener('DOMContentLoaded', async () => {
    if (!await checkRequiredConfig()) return;
    ...
    initializeGestures(player);

    // Initialize glass surface effects
    injectGlassSurfaceStyles();

    /* Glass surface effect enabled */
    ...
    document.addEventListener('click', async (e) => {
        ...
        if (e.target.closest('#smart-shuffle-album-btn')) {
            const btn = e.target.closest('#smart-shuffle-album-btn');
            if (btn.disabled) return;
            ...
            try {
                ...
                const { showNotification } = await import('./downloads.js');
                showNotification('Failed to smart shuffle album: ' + error.message);
            }
        }
        ...
        if (e.target.closest('#smart-shuffle-liked-tracks-btn')) {
            const btn = e.target.closest('#smart-shuffle-liked-tracks-btn');
            if (btn.disabled) return;
            ...
            try {
                ...
                console.error('Failed to smart shuffle liked tracks:', error);
            }
        }
    ...
    const syncSearchInputs = (sourceInput, targetInput, value) => {
    ...
    const handleSearchInput = (e) => {
        ...
        // Sync both inputs
        syncSearchInputs(e.target, searchInput, e.target.value);
        syncSearchInputs(e.target, topSearchInput, e.target.value);
        if (query.length > 2) {
            performSearch(query);
        }
    };
    ...
    if (topSearchInput) {
        topSearchInput.addEventListener('input', handleSearchInput);
    ...
    if (searchForm) {
        searchForm.addEventListener('submit', (e) => {
            e.preventDefault();
            const query = searchInput?.value.trim() || topSearchInput?.value.trim();
            if (query) {
                window.location.hash = `#search/${encodeURIComponent(query)}`;
            }
        });
    ...
    // Handle top nav button active states
    const updateTopNavActiveState = () => {
        const hash = window.location.hash || '#home';
        const navButtons = document.querySelectorAll('.top-nav-btn[data-nav]');
        navButtons.forEach((btn) => {
            const navTarget = btn.getAttribute('data-nav');
            if (hash.startsWith(`#${navTarget}`)) {
...
function showUpdateNotification(updateCallback) {
    ...
    notification.innerHTML = `
        <div>
            ...
            <p>A new version of Loop is available.</p>
        </div>
