...
export function initializeSettings(scrobbler, player, api, ui) {
    ...
    if (accentPicker) {
        ...
        if (accentCustomInput) {
            accentCustomInput.value = accentColorSettings.getCustomColor();
        }
        ...
        accentPicker.addEventListener('click', (e) => {
            ...
            // Don't change selection when clicking the color input
            if (accent === 'custom' && e.target.tagName === 'INPUT') {
                return;
            }
    ...
    document.getElementById('export-library-btn')?.addEventListener('click', async () => {
        ...
        a.href = url;
        a.download = `loop-library-${new Date().toISOString().split('T')[0]}.json`;
        a.click();
    ...
    // const customPbUrlInput = document.getElementById('custom-pb-url'); // Removed
    ...
    if (customDbBtn && customDbModal) {
        customDbBtn.addEventListener('click', () => {
            const fbConfig = localStorage.getItem('loop-firebase-config');
